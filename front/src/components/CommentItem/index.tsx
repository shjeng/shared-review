...
const CommentItem = ({comment}: Props) => {
    ...
    const timeFormat = moment(comment.createDateTime).format('YYYY. MM. DD HH:mm:ss');
                    ...
                    {comment.user.profileImage ?
                        <div className="comment-profile-img-box pointer">
                            <div className="profile-img"
                                 style={{backgroundImage: `url(${comment.user.profileImage})`}}></div>
                        </div>
                        :
                        <div className="board-detail-profile-img pointer"></div>}
                    <div className="comment-item-nickname pointer">{comment.user.nickname}</div>
                </div>
